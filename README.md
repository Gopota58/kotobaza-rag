...
База знаний про уход за рыжим котом — демонстрационный корпус (20 тематических
инструкций: питание, здоровье, туалет, безопасность и т.д.). Замените `docs/` на свои
`.txt` — и движок будет отвечать по вашей области.

## Демо

| Веб-чат | Telegram-бот |
|:--:|:--:|
| ![web demo](assets/web.gif) | ![telegram demo](assets/telegram.gif) |
...
### Результаты (n=10 золотых вопросов; LM Studio: `agents-a1-4b` + `nomic-embed-text`)
...
| Context recall (lexical) | **0.55** |

> Честно: 7 из 10 вопросов получают развёрнутые точные ответы (по ним все метрики = 1.0);
> 3 — отказ маленькой локальной модели «нет ответа» при наличии ответа в корпусе. Узкое
> место — генеративная модель, а не retrieval: с более сильной LLM relevancy/faithfulness
> заметно выше. Цифры воспроизводимы командой выше (`results.json` не коммитится).
